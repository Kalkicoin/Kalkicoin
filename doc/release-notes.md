Kalkicoin Core version *3.1.1* is now available from:  <https://github.com/kalkicoin-project/kalkicoin/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/kalkicoin-project/kalkicoin/issues>

Non-Mandatory Update
==============

Kalkicoin Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.

How to Upgrade
==============

If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Kalkicoin-Qt (on Mac) or kalkicoind/kalkicoin-qt (on Linux).


Compatibility
==============

Kalkicoin Core is extensively tested on multiple operating systems using
...
Kalkicoin Core should also work on most other Unix-like systems but is not
...
### Fix spending for v1 zKLC created before block 1050020

The transition to v2 zKLC and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zKLC to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zKLC v1 will now be able to convert that into KLC and then zKLC v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zKLC stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by Kalkicoin stakers.
...
### Show progress percent for zklc reindex operations
...
Kalkicoin Daemon & Client (RPC Changes)
--------------
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zKLC code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zKLC elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zklc reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zKLC created before block 1050020. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/kalkicoin-project-translations/).
