...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::KLC);
    ...
    if (!settings.contains("nAnonymizeKalkicoinAmount"))
        settings.setValue("nAnonymizeKalkicoinAmount", 1000);

    nAnonymizeKalkicoinAmount = settings.value("nAnonymizeKalkicoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKalkicoinAmount"))
        SoftSetArg("-anonymizekalkicoinamount", settings.value("nAnonymizeKalkicoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kalkicoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKalkicoinAmount:
            return QVariant(nAnonymizeKalkicoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKalkicoinAmount:
            nAnonymizeKalkicoinAmount = value.toInt();
            settings.setValue("nAnonymizeKalkicoinAmount", nAnonymizeKalkicoinAmount);
            emit anonymizeKalkicoinAmountChanged(nAnonymizeKalkicoinAmount);
            break;
        case CoinControlFeatures:
