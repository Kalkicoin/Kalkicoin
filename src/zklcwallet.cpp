...
CzKLCWallet::CzKLCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zklc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKLCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKLCSeed_deprecated()) {
                    LogPrintf("%s: Updated zKLC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zklc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zklc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKLCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKLCCount(nCountLastUsed);
    else if (!walletdb.ReadZKLCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKLCWallet::Lock()
{
    seedMaster = 0;
}

void CzKLCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKLCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKLC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKLCWallet::LoadMintPoolFromDB()
{
...
void CzKLCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKLCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKLCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zklcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKLCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKLC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zklcTracker which also adds to database
    pwalletMain->zklcTracker->Add(dMint, true);
...
void CzKLCWallet::SeedToZKLC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKLCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKLCWallet::GenerateDeterministicZKLC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKLCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKLCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
