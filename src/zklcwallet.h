...
#ifndef Kalkicoin_ZKLCWALLET_H
#define Kalkicoin_ZKLCWALLET_H
...
public:
    CzKLCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZKLC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZKLC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Kalkicoin_ZKLCWALLET_H
