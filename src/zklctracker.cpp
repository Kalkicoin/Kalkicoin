...
CzKLCTracker::CzKLCTracker(std::string strWalletFile)
{
...
CzKLCTracker::~CzKLCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKLCTracker::Init()
{
...
bool CzKLCTracker::Archive(CMintMeta& meta)
{
...
bool CzKLCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKLCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKLCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKLCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzKLCTracker::GetSerialHashes()
{
...
CAmount CzKLCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKLCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzKLCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzKLCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKLCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKLCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKLCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKLCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKLCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKLCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKLCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKLCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKLCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKLCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKLCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKLCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKLCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzklc from DB\n", __func__, listDeterministicDB.size());
    }
