...
// The zKLC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zKLC is a hash of the serial
...
{
    //Create an output returning the zKLC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZKLCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zKLC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zKLC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZKLCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zKLC output", __func__);
...
{
    CzKLCTracker* zklcTracker = pwallet->zklcTracker.get();
    CMintMeta meta;
    if (!zklcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zklcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a KLC stake is the outpoint
