...
public:
    ...
    bool WriteZKLCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZKLCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZKLCSeed_deprecated(uint256& seed);
    bool EraseZKLCSeed();
    bool EraseZKLCSeed_deprecated();

    bool WriteZKLCCount(const uint32_t& nCount);
    bool ReadZKLCCount(uint32_t& nCount);
