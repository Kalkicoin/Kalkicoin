...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zklcTracker = unique_ptr<CzKLCTracker>(new CzKLCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZKLC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZKLC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
